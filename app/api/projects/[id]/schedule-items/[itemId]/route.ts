...
export async function PUT(
  ...
  try {
    ...
    // Validate DPWH item exists in catalog if changed
    ...
    const itemIndex = project.scheduleItems.findIndex(item => item.id === itemId);
    if (itemIndex === -1) {
...
export async function DELETE(
  ...
  try {
    ...
    project.scheduleItems = project.scheduleItems.filter(item => item.id !== itemId);
    await project.save();
